...
class VastaanottoResource @Autowired (vtsService: VTSService) {
  ...
  def doVastaanotto(@PathVariable hakemusOid: String, @PathVariable hakukohdeOid: String): ResponseEntity[String] = {
    val result = vtsService.doVastaanotto(hakemusOid, hakukohdeOid)
